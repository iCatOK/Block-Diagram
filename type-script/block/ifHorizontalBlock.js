"use strict";
//depends: block
function blockContainsDecision(block) {
    ...
    if (block instanceof SimpleBlockOfBlocks || block instanceof HorizontalBranchBlockOfBlocks) {
        for (let inner of block.innerElements) {
            if (blockContainsDecision(inner))
                return true;
        }
    }
    return false;
...
class IfHorizontalBlock extends BlockOfBlocks {
    ...
    calculateStackedBoundingBox(compileInfo) {
        let rootElement = this.rootElement;
        let rootSize = rootElement.measureSize(compileInfo);
        let rootH = rootSize.height;
        let rootW = rootSize.width;
    ...
    compileStacked(centerXCursor, cursorY, compileInfo, myBB, rootElement, rootW, rootH, topMargin) {
        ...
        if (branchTitles != null) {
            svgResult.push(defaultCenterText(centerX + daPosition.offset.x, diamondY + rootH + daPosition.offset.y, 0, 0, branchTitles[0], daPosition.baseline, daPosition.anchor));
            svgResult.push(defaultCenterText(centerX + rootW / 2 + netPosition.offset.x, diamondY + rootH / 2 + netPosition.offset.y, 0, 0, branchTitles[1], netPosition.baseline, netPosition.anchor));
        }
        cursorY.move(rootH);
        cursorY.move(topMargin);
        ...
        compileInfo.isLast = wasLast;
        let rightWire = Math.max(centerX + rootW / 2, centerX + thenBB.bounds.right, centerX + elseBB.bounds.right) + this.marginBetweenBlocks;
        let leftWire = Math.min(centerX - rootW / 2, centerX + thenBB.bounds.left, centerX + elseBB.bounds.left) - this.marginBetweenBlocks;
        svgResult.push(makePath([
