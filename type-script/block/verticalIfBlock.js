...
class VerticalIfLayout {
    constructor(rootW, rootH, branchStart, sideOffset, downEnd, sideEnd, joinY, exitY, downEmpty, sideEmpty, downBB, sideBB) {
        this.rootW = rootW;
        this.rootH = rootH;
...
class VerticalIfBlock extends AbstractBlock {
    constructor(rootElement, downBranch, sideBranch) {
        super();
    ...
    layout(compileInfo) {
        ...
        return new VerticalIfLayout(rootSize.width, rootSize.height, branchStart, sideOffset, downEnd, sideEnd, joinY, exitY, downEmpty, sideEmpty, downBB, sideBB);
    }
    ...
    compile(centerXCursor, cursorY, compileInfo) {
        ...
        svgResult.push.apply(svgResult, this.rootElement.compile(spineX - layout.rootW / 2, y0, layout.rootW, layout.rootH));
        ...
        if (merge.length > 0)
            svgResult.push(makePath(merge));
        svgResult.push("</g>");
        cursorY.value = exitY;
        return new CompileResult(Vector.new(spineX, exitY), svgResult);
    }
    drawExitTitle(svgResult, branch, positionIndex, layout, spineX, y0) {
        ...
        svgResult.push(defaultCenterText(position.x + titlePosition.offset.x, position.y + titlePosition.offset.y, 0, 0, branch.title, titlePosition.baseline, titlePosition.anchor));
    }
    pushSegment(lines, x1, y1, x2, y2) {
        if (Math.abs(x1 - x2) < 0.5 && Math.abs(y1 - y2) < 0.5)
            return;
        lines.push(rawSvgLine(x1, y1, x2, y2));
    }
    pushOntoSpine(lines, x, y, spineX, joinY) {
...
class VerticalElifChain extends AbstractBlock {
    ...
    drawDiamondTitles(svgResult, spineX, diamondY, diamondW, diamondH) {
        const titles = ["", "Нет", "Да"];
        for (let index = 1; index <= 2; index++) {
            ...
            svgResult.push(defaultCenterText(position.x + titlePosition.offset.x, position.y + titlePosition.offset.y, 0, 0, titles[index], titlePosition.baseline, titlePosition.anchor));
        }
    }
    pushSegment(lines, x1, y1, x2, y2) {
        if (Math.abs(x1 - x2) < 0.5 && Math.abs(y1 - y2) < 0.5)
